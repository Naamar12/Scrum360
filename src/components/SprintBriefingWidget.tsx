...
function BacklogImportPopup({
  ...
  const available = backlogIssues.filter(i => !currentTexts.has(i.summary.trim()));

  return (
    <div className="absolute left-0 bottom-6 z-30 bg-[#2c2c2c] border border-[#404040] rounded-xl shadow-2xl p-2 w-64 max-h-52 flex flex-col gap-1">
...
function DraggableCard({
  ...
  if (compact) {
    return (
      <div
        data-card-id={devKey}
        className="bg-[#252525] border border-[#353535] rounded-xl group/card overflow-hidden"
      >
  ...
  return (
    <div
      data-card-id={devKey}
      className="bg-[#252525] border border-[#353535] rounded-2xl flex flex-col group/card overflow-hidden"
    >
      {/* Header – distinct background */}
      <div className="bg-[#2c2c2c] border-b border-[#353535] px-3 py-2 flex items-center gap-2">
        <GripVertical
          className="w-4 h-4 text-[#444] cursor-grab active:cursor-grabbing shrink-0 opacity-0 group-hover/card:opacity-100 transition-opacity"
          onMouseDown={() => onCardGripMouseDown?.()}
          onMouseUp={() => onCardGripMouseUp?.()}
